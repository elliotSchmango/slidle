...
class Game {
    ...
    constructor(difficultyLevel = 1) {
        this.difficulty = GameDiff[difficultyLevel - 1];
    ...
    onClickOnBlock(blockIdx) { //try move block and check if puzzle was solved
        if (this.moveBlock(blockIdx)) {
            if (this.checkPuzzleSolved()) {
                setTimeout(() => alert("Puzzle Solved In " + (moveCounter - 30) + " Moves!!!"), 600);
            }
        }
    }
        ...
        for (let i = 0; i < this.indexes.length; i++) {
            ...
            for (let y = 0; y < this.rows; y++) {
                for (let x = 0; x < this.cols; x++) {
                    ...
                    if (blockIdx = a) {
                        block.innerHTML = `
                        <div class="puzzle-block" style="background-color:green"</div>
                    `
                    }
                    a++;
                }
...
difficulty_buttons.forEach((elem, idx) => {
    elem.addEventListener('click', (e) => {
        difficulty_buttons[GameDiff.indexOf(game.difficulty)].classList.remove("active");
